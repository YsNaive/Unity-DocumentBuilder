...
namespace NaiveAPI.DocumentBuilder
{
    public class DocSeeAlso : DocVisual
    {
        ...
        protected override void OnCreateGUI()
        {
            ...
            scrollView.style.marginLeft = 2 * SODocStyle.Current.MainTextSize;
            VisualElement mask = DocRuntime.NewEmpty();
            mask.pickingMode = PickingMode.Ignore;
            mask.style.position = Position.Absolute;
            mask.style.marginLeft = 2 * SODocStyle.Current.MainTextSize;
            Color maskColor = SODocStyle.Current.HintColor;
            maskColor.a = 0.25f;
            mask.style.backgroundColor = maskColor;
            VisualElement child = new VisualElement();
            child.style.SetIS_Style(ISFlex.Horizontal);
            child.style.paddingLeft = SODocStyle.Current.MainTextSize;
            ...
            button.style.SetIS_Style(ISMargin.Pixel(SODocStyle.Current.MainTextSize / 2));
            TextElement textElement = DocRuntime.NewTextElement(Target.TextData[0]);
            textElement.style.SetIS_Style(ISMargin.Pixel(SODocStyle.Current.MainTextSize / 2));
            child.Add(textElement);
            child.Add(button);
            this.style.borderLeftWidth = SODocStyle.Current.MainTextSize / 2f;
            this.style.borderLeftColor = SODocStyle.Current.FrontgroundColor;
            this.Add(child);
            this.Add(scrollView);
            this.Add(mask);
